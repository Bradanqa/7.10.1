...
{
    ui->tb_result->append(QString("Время на сервере: ") + time.toString());
}
void MainWindow::DisplayFreeSpace(uint32_t freeSpace)
{
    ui->tb_result->append(QString("Free space = ") + QString::number(freeSpace));
}
void MainWindow::SetDataReply(QString replyString)
{
    ui->tb_result->append(QString("User data: ") + replyString);
}
void MainWindow::DisplayStat(StatServer stat)
{
    ui->tb_result->append(QString("Server Stat: incBytes: %1, sendBytes: %2, revPck: %3, sendPck: %4, workTime: %5, clients: %6")
            .arg(stat.incBytes)
            .arg(stat.sendBytes)
            .arg(stat.revPck)
            .arg(stat.sendPck)
            .arg(stat.workTime)
            .arg(stat.clients));
                                    //.. добавить в класс клиента учет статистики по каждому пункту
}
